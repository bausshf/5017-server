module conquer.security.auth.servercryptographer;

import conquer.security.cryptographer;
import conquer.security.key;

private {
  /// The first keys.
  enum ubyte[] keys1 = [
    
  ];

  /// The secondary keys.
  enum ubyte[] keys2 = [
    
  ];
}

/// Wrapper around the server Cryptographer for the auth server.
final class ServerCryptographer : Cryptographer {
  private:
  /// The first keys.
  immutable(ubyte[]) _keys1;

  /// The second keys;
  immutable(ubyte[]) _keys2;

  /// The encrypt counter.
  Key!ushort _encryptCounter;

  /// The decrypt counter.
  Key!ushort _decryptCounter;

  public:
  final:
  /// Creates a new server cryptographer.
  this() {
    _keys1 = keys1.idup;
    _keys2 = keys2.idup;
  }

  /**
  * Encrypts a buffer.
  * Params:
  *   buffer = The buffer to encrypt.
  */
  override void encrypt(ubyte[] buffer) pure @safe {
    foreach (i; 0 .. buffer.length) {
      auto value = buffer[i];

      value = cast(ubyte)(value ^ 0xAB);
      value = cast(ubyte)((value << 4) | (value >> 4));
      value = cast(ubyte)(_keys2[_encryptCounter >> 8] ^ value);
      value = cast(ubyte)(_keys1[_encryptCounter & 0xFF] ^ value);

      buffer[i] = value;

      _encryptCounter.increment();
    }
  }

  /**
  * Decrypts a buffer.
  * Params:
  *   buffer = The buffer to decrypt.
  */
  override void decrypt(ubyte[] buffer) pure @safe {
    foreach (i; 0 .. buffer.length) {
      auto value = buffer[i];

      value = cast(ubyte)(value ^ 0xAB);
      value = cast(ubyte)((value << 4) | (value >> 4));
      value = cast(ubyte)(keys2[_decryptCounter >> 8] ^ value);
      value = cast(ubyte)(keys1[_decryptCounter & 0xFF] ^ value);

      buffer[i] = value;

      _decryptCounter.increment();
    }
  }
}
